...
if st.button('Générer le résumé'):
    if not files:
        ...
        with st.spinner('Génération...'):
            try:
                res = get_client().chat.completions.create(model='mistralai/mistral-small-24b-instruct', messages=[{'role':'user','content':prm}], temperature=0.2, top_p=0.7, max_tokens=2048, stream=True)
                out = ''
                ph = st.empty()
                for c in res:
                ...
                st.download_button('Télécharger PDF', data=generate_pdf(out), file_name='resume.pdf', mime='application/pdf')
            except Exception as e:
